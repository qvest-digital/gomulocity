...
func TestClient_CreateDeviceRegistration(t *testing.T) {
	tests := []struct {
		name                       string
	...
	}{
		{
			...
			expectedErr:            DeviceRegistrationAlreadyExistsErr,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			newDeviceRequest, err := c.CreateDeviceRegistration(tt.newDeviceRequestID)
			if fmt.Sprint(err) != fmt.Sprint(tt.expectedErr) {
				t.Fatalf("respond with unexpected error. Expected: %s. Given: %s", tt.expectedErr, err)
			}

			if !reflect.DeepEqual(newDeviceRequest, tt.expectedDeviceRegistration) {
				t.Errorf("respond with unexpected newDeviceRequest. \nExpected: %#v. \nGiven: %#v", tt.expectedDeviceRegistration, newDeviceRequest)
			}
...
func TestClient_DeviceRegistrations(t *testing.T) {
	tests := []struct {
		name                                 string
		...
		expectedDeviceRegistrationCollection DeviceRegistrationCollection
		expectedErr                          error
	}{
		{
			name:               "happy case",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			newDeviceRequest, err := c.DeviceRegistrations(tt.reqOpts)
			if fmt.Sprint(err) != fmt.Sprint(tt.expectedErr) {
				t.Fatalf("respond with unexpected error. Expected: %s. Given: %s", tt.expectedErr, err)
			}

			if !reflect.DeepEqual(newDeviceRequest, tt.expectedDeviceRegistrationCollection) {
				t.Errorf("respond with unexpected newDeviceRequestCollection. \nExpected: %#v. \nGiven: %#v", tt.expectedDeviceRegistrationCollection, newDeviceRequest)
			}
...
func TestClient_UpdateDeviceRegistration(t *testing.T) {
	tests := []struct {
		name                       string
		newDeviceRequestID         string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			newDeviceRequest, err := c.UpdateDeviceRegistration(tt.newDeviceRequestID, tt.newDeviceRequestStatus)
			if fmt.Sprint(err) != fmt.Sprint(tt.expectedErr) {
				t.Fatalf("respond with unexpected error. Expected: \n%s. Given: \n%s", tt.expectedErr, err)
			}

			if !reflect.DeepEqual(newDeviceRequest, tt.expectedDeviceRegistration) {
				t.Errorf("respond with unexpected newDeviceRequest. Expected: %#v. Given: %#v", tt.expectedDeviceRegistration, newDeviceRequest)
			}
...
func TestClient_DeleteDeviceRegistration(t *testing.T) {
	tests := []struct {
		name               string
		newDeviceRequestID string
		c8yRespCode        int
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := c.DeleteDeviceRegistration(tt.newDeviceRequestID)
			if fmt.Sprint(err) != fmt.Sprint(tt.expectedErr) {
				t.Fatalf("respond with unexpected error. Expected: %s. Given: %s.", tt.expectedErr, err)
			}
